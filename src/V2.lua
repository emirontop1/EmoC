...
function EmoC:CreateWindow(mainTitle, subTitleText, config)
	...
	local confirmClose = config.confirmClose == nil and true or config.confirmClose -- Varsayılan true
	...
	-- Dinamik Tema Kayıt Sistemi (Renkleri anlık değiştirebilmek için)
	local ThemeRegistry = { Accent = {}, Stroke = {}, Background = {} }
	...
	-- YENİ: RESIZABLE SIDEBAR (Hareketli Orta Çizgi)
	...
	UserInputService.InputChanged:Connect(function(input)
		if divResizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			...
			currentSidebarWidth = math.clamp(divStartWidth + delta.X, 100, 250) -- Sidebar Genişlik Sınırları
	...
	-- YENİ: POPUP SİSTEMİ (Ortada Çıkan Uyarı)
	...
	-- Window Fonksiyonlarını Objeye Ekleme
	...
	-- Geri Kalan Sürükleme ve Boyutlandırma (Öncekiyle Aynı)
	...
	local function BuildElements(ParentContainer)
		...
		-- YENİ: DROPDOWN SİSTEMİ (Clipping sorunu çözülmüş absolute position)
		function Elements:CreateDropdown(name, options, default, callback)
			...
			-- Dropdown Listesi ScreenGui içine atılıyor ki ScrollingFrame içinde kesilmesin!
			...
			DropBtn.MouseButton1Click:Connect(function()
				...
				if isOpen then
					RenderOptions()
					-- Butonun tam pozisyonunu alıp ekranın üstüne çiziyoruz
			...
			-- GUI hareket ettiğinde açık olan dropdown'u kapat
